##' ---
##' title: "Assemble/extract baseline variables for matching"
...
##' ## Population
##'
##' Read in the data and filter it down to
##' the "prereform period".  For Sommers, Long and Baicker
##' (_Ann. Intern. Med._ 2014; henceforth "SLB")
##' this was either 2001-2005 or 2000-2006 (cf note accompanying
##' Appendix Table 2, propensity score coefs; p.7).
##' For us, 2009-2013, inclusive (I think). 
##'
##' Values for the Record type variable, per
##' file w/ specs (1999-2013CMFDocumentation.docx):
##'
##'  1 National population record
##'  2 State pop record
##'  3 County pop record
##'
##' This being an assembling of baseline data, we
##' immediately filter out observations 
##' following the baseline period. 
#+ popreadin, eval=TRUE, cache=2
...
##' Mutate the variables to required forms, discarding
##' what neither appears in their Appendix Table 2
##' nor looks otherwise essential to our purposes.
##'
...
##'
##' Here we create race and gender variables, also
##' mutate `race_sex` by collapsing racial categories
##' other than B. & W.
##' (NB: the code solution I'm using here requires
##' R version 3.5.0, per
##' ["tim" on StackOverflow](https://stackoverflow.com/questions/19410108/cleaning-up-factor-levels-collapsing-multiple-levels-labels) .)
...
##' 
##' Here `ax_y` records population in age group ranging from x to y.
##' SLB merged some age categories. 
##' (They also say they weighted logistic regressions for
##' "size of each county’s non-elderly adult population.")
##' 
#+ pop_age_reduce
...
##'
##' For age-standardization within county and race/sex
##' subcategory, we need the age distribution across
##' county, race and sex.  We'll follow WONDER in using the 2000 distribution.
...
##' The table `pop_tall` has distinct rows not only for distinct
##' counties and/or years, but also for distinct values of gender, race
##' and ethnicity variables.
##'
##' First marginalize over race, hispanic origin, gender and age, but leave
##' out interaction categories. 
#+
...
##' Now create version of `pop_tall` collapsing cells that
##' differ only by hispanic origin. 
#+
...
##' 
##' SLB weight their propensity scores for population size. 
##' We'll follow, using the average of 2008--2013 population sizes. 
#+
margins_by_year  %>% filter(2009<=year, year<=2013) %>%
    ungroup() %>%  group_by(stateFIPS, cntyFIPS) %>%
...
##' (The averages of 2009-13 adult population sizes being
##' `adult_w_a_cnt` and `adult_all_cnt`, respectively, for
##' working age adults and for all adults.
##' The `adjcnt` version standarizes each age distribution to
##' that of the country, and doesn't seem to play a role for SLB.)
##' Lagged variant:
margins_by_year  %>% filter(2008<=year, year<=2012) %>%
    ungroup() %>%  group_by(stateFIPS, cntyFIPS) %>%
    summarise(adult_w_a_cnt=mean(adult_w_a_cnt),
...
##' ## Mortality
##'
##' Again we read in only what looks important for
##' baseline analyses paralleling those of
##' SLB.
#+ mortreadin, cache=2
...
##' Save the mortality cross-tabulation after aggregating over the full
##' pre-intervention period. (Keeping us in line with the "aggregate
##' over at least 3 years" rule.)
##' For the purpose of capturing codes of death in a shareable file.
#+ codesofdeathreadout, cache=2, dependson="mortreadin"
...
##'
##' It so happens that several of the aggregated cause-of-death
##' designations of interest to us are neatly nested.
#+ aggcod, cache=2, dependson=c("setup1","codesofdeathreadout")
...
##' ### Reshaping
##' 
##' First rewrite race_sex variable, collapsing
##' races other than B/W as in SLB, as well as
...
##' Reduce the table to categories we're likely to use, 
##' reshape as necessary for joining with demographics &
##' economics tables. 
#+
mort_tall %>% 
...
##' Now join up the restructured tables. (Before merging I record row
##' counts, so I'll be able to identify unexpected increase.)
#+
...
margins_by_year$mortAC_20_64 <- 1e5*
    margins_by_year$dead20_64/margins_by_year$adult_w_a_cnt
margins_by_year %>% filter(year>=2009, year<=2013) %>%
    select(stateFIPS, cntyFIPS, year, mortAC_20_64) %>%
    pivot_wider(names_from=year, values_from=mortAC_20_64,
                names_prefix="mortAC") %>% 
    right_join(base_cnty, by = c("stateFIPS", "cntyFIPS")
               ) -> base_cnty
base_cnty %>% select(starts_with("mortAC20")) %>%
...
agesY2K_w_a
##' 
##' Here we figure mortality rates by age group, for each combination of
##' county, sex and gender, and go on immediately to aggregate
##' across age groups using proportions given above.
##' Also mortality rates by age group, for each
##' cause-of-death and county, aggregated by age groups using
##' precisely the same proportions.
##' Again we use a 5 year window, then separately perform parallel
##' calculations for the 5 year window lagged one year back.
##' Preceding underscores again mark the lagged versions. 
#+  
...
##' ... and parallel calculations for the one-year-lagged version,
##' `.base_cnty` (code suppressed).
#+ echo=FALSE
...
c_o_d_cat_by_year %>% filter(year>=2008, year<=2012) %>% 
    select(stateFIPS, cntyFIPS, c_o_d_cat, dead20_34,
           dead35_44, dead45_54, dead55_64,
           a20_34:a55_64, adult_w_a_cnt, adult_all_cnt) %>%
    group_by(stateFIPS, cntyFIPS, c_o_d_cat) %>%
...
##' The survey package has a glm function that interprets
##' weights as SLB did. Below is the call I used to replicate their PS
##' using earlier data:
##' 
#+ eval=FALSE
