##' ---
##' title: "Assemble/extract population variables for 2005-2014"
...
pop_clean <- pop_tall %>% 
  ...
  filter(record_type==3, year %in% 2005:2014) %>%
  select(-births,-record_type, -race_sex, -hisp_code) %>%
  pivot_longer(a65_74:a85plus, names_to = "age", values_to = "pop", names_prefix = "a") %>%
  group_by(stateFIPS, cntyFIPS, year, race, age) %>%
  summarize(pop = sum(pop)) %>%
  filter(!(race == "Other" & pop == 0))


save(pop_clean, file = file.path("..", "data","pop_65.Rdata"))
